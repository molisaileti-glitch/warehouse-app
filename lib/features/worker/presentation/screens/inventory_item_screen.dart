...
class InventoryItemScreen extends ConsumerWidget {
  ...
  const InventoryItemScreen(
      {super.key, required this.warehouseId, required this.itemId});
    ...
    return Scaffold(
      appBar: AppBar(
        title: itemAsync.maybeWhen(
            data: (i) => Text(i?.name ?? ''), orElse: () => Text(l10n.item)),
        actions: [
          itemAsync.maybeWhen(
            data: (item) => item != null
                ? IconButton(
                    ...
                    onPressed: () async {
                      final ok = await showConfirmDialog(context,
                          ...
                          message: l10n.deleteItemConfirm(item.name),
      ...
      floatingActionButton: FloatingActionButton.extended(
        ...
        label: Text(l10n.record, style: const TextStyle(color: Colors.white)),
      ),
      body: itemAsync.when(
        data: (item) {
          if (item == null) return ErrorView(message: l10n.itemNotFound);
          return CustomScrollView(
            slivers: [
                    ...
                    children: [
                      ...
                      Row(children: [
                        _HeaderStat(
                            label: l10n.onHand,
                            value:
                                '${item.quantityOnHand % 1 == 0 ? item.quantityOnHand.toInt() : item.quantityOnHand.toStringAsFixed(1)}',
                            unit: item.unit),
                        const SizedBox(width: 20),
                        _HeaderStat(
                            label: l10n.reorderAt,
                            value:
                                '${item.reorderLevel % 1 == 0 ? item.reorderLevel.toInt() : item.reorderLevel}',
                            unit: item.unit),
              ...
              SliverToBoxAdapter(
                  child: SectionHeader(title: l10n.movementHistory)),
              movementsAsync.when(
                data: (movements) {
...
class _MovementTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final fmt = DateFormat('MMM d • HH:mm', l10n.localeName);
    ...
    return AppCard(
      ...
      child: Row(children: [
          ...
          children: [
            Text(
                switch (type) {
                  ...
                  'transfer_out' => l10n.transferOut,
                  'transfer_in' => l10n.transferIn,
                  'adjustment' => l10n.adjustment,
                  'count' => l10n.stockCount,
                  _ => l10n.movement,
                },
                style: const TextStyle(
                    ...
                    fontSize: 12,
                    letterSpacing: 0.3)),
            Text(fmt.format(movement.createdAt),
                style:
        ...
        Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
          ...
          Text(
              l10n.previousQuantity(
                  '${before % 1 == 0 ? before.toInt() : before.toStringAsFixed(1)}'),
...
class _RecordMovementSheet extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<_RecordMovementSheet> createState() =>
      _RecordMovementSheetState();
}
...
class _RecordMovementSheetState extends ConsumerState<_RecordMovementSheet> {
  ...
  final _types = [
    ('delivery', Icons.arrow_downward_rounded, AppColors.success),
    ('adjustment', Icons.tune_rounded, AppColors.workerColor),
    ('count', Icons.checklist_rounded, AppColors.info),
  ];
            ...
            return Expanded(
              ...
              child: GestureDetector(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    color: selected ? t.$3.withOpacity(0.1) : AppColors.surface,
                    borderRadius: BorderRadius.circular(10),
                    border: Border.all(
                        color: selected ? t.$3 : AppColors.divider,
                        width: selected ? 1.5 : 1),
                  ),
                  child: Column(children: [
                    Icon(t.$2,
                        color: selected ? t.$3 : AppColors.textMuted, size: 20),
                    const SizedBox(height: 4),
                    Text(
                        switch (t.$1) {
                          'delivery' => l10n.delivery,
                        ...
                        style: TextStyle(
                            fontSize: 10,
                            color: selected ? t.$3 : AppColors.textMuted,
                            fontWeight: selected
                                ? FontWeight.w700
                                : FontWeight.normal)),
                  ]),
